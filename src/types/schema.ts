...
export class FoxchainDayData extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id !== null, "Cannot save FoxchainDayData entity without an ID");
    assert(
      id.kind == ValueKind.STRING,
      "Cannot save FoxchainDayData entity with non-string ID. " +
        'Considering using .toHex() to convert the "id" to a string.'
    );
    store.set("FoxchainDayData", id.toString(), this);
  }

  static load(id: string): FoxchainDayData | null {
    return store.get("FoxchainDayData", id) as FoxchainDayData | null;
  }
