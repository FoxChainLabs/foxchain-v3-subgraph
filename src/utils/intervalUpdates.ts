 ...
 * @param event
 */
export function updateFoxchainDayData(event: ethereum.Event): FoxchainDayData {
  let foxchain = Factory.load(FACTORY_ADDRESS)
  let timestamp = event.block.timestamp.toI32()
  let dayID = timestamp / 86400 // rounded
  let dayStartTimestamp = dayID * 86400
  let foxchainDayData = FoxchainDayData.load(dayID.toString())
  if (foxchainDayData === null) {
    foxchainDayData = new FoxchainDayData(dayID.toString())
    foxchainDayData.date = dayStartTimestamp
    foxchainDayData.volumeETH = ZERO_BD
    foxchainDayData.volumeUSD = ZERO_BD
    foxchainDayData.volumeUSDUntracked = ZERO_BD
    foxchainDayData.feesUSD = ZERO_BD
  }
  foxchainDayData.tvlUSD = foxchain.totalValueLockedUSD
  foxchainDayData.txCount = foxchain.txCount
  foxchainDayData.save()
  return foxchainDayData as FoxchainDayData
}
